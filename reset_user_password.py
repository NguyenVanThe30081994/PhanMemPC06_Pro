...
def main():
    ...
    try:
        ...
        print("Không thể kết nối database hiện tại.", file=sys.stderr)
        print(f"- db_uri: {db_uri}", file=sys.stderr)
        print(f"- lỗi gốc: {exc}", file=sys.stderr)
        print(
            "- kiểm tra lại DATABASE_URL trên server; nếu đang dùng chuỗi mẫu như "
            "'mysql://user:pass@host/dbname' thì phải thay host thật, thường là 'localhost'.",
            file=sys.stderr,
        )
        raise SystemExit(2)
