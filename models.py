...
class Task(db.Model):
    ...
    parent_task_id = db.Column(db.Integer, db.ForeignKey('task.id'), index=True)
...
class TaskAssignment(db.Model):
    ...
    status = db.Column(db.String(50), default='Chưa tiếp nhận', index=True)
    is_required = db.Column(db.Boolean, default=True)
    result_file = db.Column(db.String(255))
    report_payload_json = db.Column(db.Text)
    assigned_at = db.Column(db.DateTime, default=datetime.now, index=True)
