...
# ==================== REPORT V3 MODELS (Metadata-Driven) ====================
# Theo cau truc tu V3Excel.md - 5 Module Architecture:
# 1. Template Importer - Doc Excel, xu li merged cells
# 2. Schema Builder - Sinh field definitions tu header phuc tap  
...
class ReportTemplateV3(db.Model):
    ...
    name = db.Column(db.String(255), nullable=False)
    description = db.Column(db.Text)
    template_code = db.Column(db.String(50), unique=True)  # Ma template duy nhat
    created_by = db.Column(db.String(100))
    created_at = db.Column(db.DateTime, default=datetime.now)
    updated_at = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)
    is_active = db.Column(db.Boolean, default=True)
    fields = db.relationship('ReportTemplateFieldV3', backref='template', cascade='all, delete-orphan')
    versions = db.relationship('ReportVersionV3', backref='template', cascade='all, delete-orphan')


class ReportTemplateFieldV3(db.Model):
    """Schema field definitions - Sinh tu header phuc tap cua Excel"""
    __tablename__ = 'report_template_field_v3'
    id = db.Column(db.Integer, primary_key=True)
    template_id = db.Column(db.Integer, db.ForeignKey('report_template_v3.id'))
    field_code = db.Column(db.String(100), nullable=False)  # snake_case: doanh_thu__quy_1__thuc_hien
    field_name = db.Column(db.String(255))  # Ten hien thi
    header_path = db.Column(db.Text)  # JSON: ["Doanh thu", "Quy 1", "Thuc hien"]
    column_index = db.Column(db.Integer)  # Vi tri cot (1-indexed)
    data_type = db.Column(db.String(20), default='text')  # number, text, date
    editable = db.Column(db.Boolean, default=True)
    required = db.Column(db.Boolean, default=False)
    default_value = db.Column(db.Text)
    validation_rules = db.Column(db.Text)  # JSON: {min, max, pattern}
    created_at = db.Column(db.DateTime, default=datetime.now)


class ReportVersionV3(db.Model):
    """Template version - Luu schema JSON va file Excel goc"""
    __tablename__ = 'report_version_v3'
    id = db.Column(db.Integer, primary_key=True)
    template_id = db.Column(db.Integer, db.ForeignKey('report_template_v3.id'))
    version_tag = db.Column(db.String(20))
    schema_json = db.Column(db.Text)  # Full schema JSON theo cau truc trong md
    excel_blob = db.Column(db.LargeBinary)  # File Excel template goc
    header_row_count = db.Column(db.Integer, default=3)  # So dong header
    created_at = db.Column(db.DateTime, default=datetime.now)
    created_by = db.Column(db.String(100))
    is_published = db.Column(db.Boolean, default=False)
    submissions = db.relationship('ReportSubmissionV3', backref='version', cascade='all, delete-orphan')


class ReportSubmissionV3(db.Model):
    """Mot lan nop bao cao - gan voi 1 don vi, 1 ky"""
    __tablename__ = 'report_submission_v3'
    id = db.Column(db.Integer, primary_key=True)
    version_id = db.Column(db.Integer, db.ForeignKey('report_version_v3.id'))
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    unit_id = db.Column(db.String(100), nullable=False)  # Don vi (phan quyen theo day)
    period_id = db.Column(db.String(50))  # Ky bao cao
    ...
    audits = db.relationship('ReportAuditV3', backref='submission', cascade='all, delete-orphan')


class ReportValueV3(db.Model):
    """Gia tri cua mot o - Luu theo field_code"""
    __tablename__ = 'report_value_v3'
    id = db.Column(db.Integer, primary_key=True)
    submission_id = db.Column(db.Integer, db.ForeignKey('report_submission_v3.id'))
    field_id = db.Column(db.Integer, db.ForeignKey('report_template_field_v3.id'))
    field_code = db.Column(db.String(100))
    value = db.Column(db.Text)


class ReportAuditV3(db.Model):
    """Audit log - Theo doi thay doi"""
